...
- **Site metadata** (name, role, socials, Cloudflare Web Analytics token, avatar path): `src/data/site.ts`.
- **Theming**: CSS custom properties in `src/styles/global.css` (`:root` light, `[data-theme='dark']` overrides). The theme is set pre-paint by an `is:inline` script in `src/layouts/Base.astro` — keep that inline or dark mode will flash. That script also sets `[data-theme-pref]` (`light` / `dark` / `system`; no `theme` in localStorage means system), which `ThemeToggle.astro` reads via CSS to pick its icon and label. The look is neo-brutalist: ink outlines (`--ink`, `--bw`), hard offset shadows and four brights (`--yellow`, `--pink`, `--cyan`, `--green`); add `.fill-cycle` to a list to rotate `--fill` through them per child, and `.brutal-btn` for the outlined press-in button. Text on a bright fill is always `--on-fill` (black), never `--accent`, to stay AA. Code blocks use `--pre-bg` (dark in both themes) because the Shiki theme is `github-dark`.
- **URL conventions**: everything is served with trailing slashes (`build.format: 'directory'`); write internal links with trailing slashes. Old Jekyll routes (`/categories/`, `/year-archive/`, `/sitemap.xml`) are redirected in `astro.config.mjs`; `/tags/` is a real page, not a redirect.
