...
- **Theming**: CSS custom properties in `src/styles/global.css` (`:root` light, `[data-theme='dark']` overrides). The theme is set pre-paint by an `is:inline` script in `src/layouts/Base.astro` — keep that inline or dark mode will flash.
